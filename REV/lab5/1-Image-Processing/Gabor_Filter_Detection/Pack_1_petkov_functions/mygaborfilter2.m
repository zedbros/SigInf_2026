function result = mygaborfilter2(img, lambda, sigma, theta, phi, gamma, bandwidth)
...
filterkernel = mygaborkernel2d(lambda, sigma, theta(1), phi(1), gamma, bandwidth); % called only to determine the size of the filterkernel
...
while (cnt1 <= nrtheta)
  ...
  while (cnt2 <= nrphi) % for each phi the convolution is calculated
    filterkernel = mygaborkernel2d(lambda, sigma, theta(cnt1), phi(cnt2), gamma, bandwidth);
    result(:,:,cnt2,cnt1) = convolution(img, filterkernel, fftimage);
    
    if cnt2==1&&cnt1==1
    figure()
    imagesc(filterkernel)
    axis image
